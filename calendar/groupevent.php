<?php 
		...
		$month==8 or $month==9 or $month==10 or $month==11 or $month==12)){
			...
			for($i=0; $i < count($name); $i++){
				if($name[$i]==null){
					echo json_encode(array(
						"success" => false,
						"message" => "All the names can not be null!"
					));
					exit;
			    }
			}
	...
	while($k < count($name)){
		...
		$stmt->bind_param('s', $name[$k]);
		$stmt->execute();
		$stmt->bind_result($result);
		$stmt->fetch();
		if (!htmlspecialchars($result)) {
			echo json_encode(array(
				"success" => false,
				"message" => "name(s) doe not exist!"
			));
			exit;
		}
		$stmt->close();
	...
	$m=0;
	while($m < count($name)){
	    $stmt = $mysqli->prepare("insert into events (username, title, year, month, date, content, tag, hour, groups, member) values (?, ?, ?, ?, ?, ?, ?, ?, 1, ?)");
	    if(!$stmt){
		    printf("Query Prep Failed: %s\n", $mysqli->error);
		    exit;
	    }
	    $stmt->bind_param('ssiiissis',$name[$m], $title, $year, $month, $date, $content, $tag, $hour, $username);
	    $stmt->execute();
	    $stmt->close();
		$m++;
	}
	...
	$stmt->bind_param('ssiiissis',$username, $title, $year, $month, $date, $content, $tag, $hour, $username);
	$stmt->execute();
	$stmt->close();
