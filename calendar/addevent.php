<?php 
		...
		$stmt = $mysqli->prepare("insert into events (username, title, year, month, date, content, tag, hour) values (?, ?, ?, ?, ?, ?, ?, ?)");
		if(!$stmt){
			printf("Query Prep Failed: %s\n", $mysqli->error);
			exit;
		}
		$stmt->bind_param('ssiiissi',$username, $title, $year, $month, $date, $content, $tag, $hour);
		$stmt->execute();
		if(true){
		echo json_encode(array(
			"success" => true
